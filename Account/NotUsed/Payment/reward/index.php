                                                    ...
                                                    <div class="panel panel-default">                                                        
                                                        <div class="panel-body">
                                                            <div class="form-group">
                                                                <label class="col-sm-3 control-label">Formula <span class="asterisk">*</span></label>
                                                                <div class="col-sm-9">
                                                                    <select class="form-control btn-block" name='reward' required style="border-left: 1px solid #ccc;border-radius: 3px">
                                                                        <?php
                                                                        $query = mysql_query("select * from p_reward where merchant_id='" . $_SESSION['merchant'] . "' and deleted=0");
                                                                        while ($row = mysql_fetch_array($query)) {
                                                                            if ($row['max'] == 0)
                                                                                echo "<option value='" . $row['id'] . "'>" . $row['name'] . " (No Max)" . "</option>";
                                                                            else
                                                                                echo "<option value='" . $row['id'] . "'>" . $row['name'] . " (Max " . $row['max'] . ")" . "</option>";
                                                                        }
                                                                        ?>
                                                                    </select>
                                                                </div>
...
<!--                                                <th class='no-sort text-center' width='180'>Action</th>-->
                                        ...
                                        <tbody>
                                            <?php
                                            $no = 0;
                                            $query = mysql_query("select *, a.id as id, a.point as point from log_reward a, customer b where
                                                a.customer_id=b.id and
                                                a.merchant_id='" . $_SESSION['merchant'] . "'  
                                                order by a.id desc");
                                            while ($row = mysql_fetch_array($query)) {
                                                $no++;
                                                ?>
